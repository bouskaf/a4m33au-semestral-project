...
fof(at_restr, axiom, (![T, Train, N]: (at(T, Train, N) => ((N = s3) | (N = out2) | (N = out3) | (N = s4) | (N = out4) | (N = in) | (N = out1) | (N = s1) | (N = s2))))).

% Only input nodes can be open.
fof(open_restr, axiom, (![T, N]: (open(T, N) => input(N)))).

% Only true for input nodes.
fof(input, axiom, (![N]: (input(N) => ((N = s4) | (N = in))))).
...
fof(distinct_nodes, axiom, ((s3 != out2) & (s3 != out3) & (s3 != s4) & (s3 != out4) & (s3 != in) & (s3 != out1) & (s3 != s1) & (s3 != s2) & (out2 != out3) & (out2 != s4) & (out2 != out4) & (out2 != in) & (out2 != out1) & (out2 != s1) & (out2 != s2) & (out3 != s4) & (out3 != out4) & (out3 != in) & (out3 != out1) & (out3 != s1) & (out3 != s2) & (s4 != out4) & (s4 != in) & (s4 != out1) & (s4 != s1) & (s4 != s2) & (out4 != in) & (out4 != out1) & (out4 != s1) & (out4 != s2) & (in != out1) & (in != s1) & (in != s2) & (out1 != s1) & (out1 != s2) & (s1 != s2))).

% Restriction for occupied values.
fof(occupied_values, axiom, (![T, Train, N]: (occupied(T, Train, N) => ((N = s3) | (N = out2) | (N = out3) | (N = s4) | (N = out4) | (N = in) | (N = out1) | (N = s1) | (N = s2))))).

% Train can enter only in input nodes.
fof(enter_values, axiom, (![T, Train, N]: (enter(T, Train, N) => ((N = s4) | (N = in))))).
...
fof(switch_s3_values, axiom, (![T]: ((switch(T, s3) = out3)))).
fof(switch_s4_values, axiom, (![T]: ((switch(T, s4) = out4)))).
...
fof(switch_s4_with_gate_out4, axiom, (![T, Train]: ((occupied(T, Train, s4) & (gate(Train) = out4)) => (switch(T, s4) = out4)))).
...
% Moves in inputs.
fof(enter_or_wait_at_s4, axiom, (![T, Train]: (enter(T, Train, s4) | (at(T, Train, s4) & ~open(T, s4))) => at(succ(T), Train, s4))).
fof(move_or_stay_from_s4_to_out4, axiom, (![T, Train]: (((at(T, Train, s4) & open(T, s4) & move(T, Train)) => (at(succ(T), Train, out4))) | ((at(T, Train, s4) & open(T, s4) & ~move(T, Train)) => at(succ(T), Train, s4))))).
...
fof(s3_occupied, axiom, (![T, Train]: (occupied(succ(T), Train, s3) <=> ((occupied(T, Train, s3) & ~(at(T, Train, s3) & ~at(succ(T), Train, s3))) | (at(T, Train, in) & (gate(Train) = out3) & open(T, in)))))).
...
fof(out4_occupied, axiom, (![T, Train]: (occupied(succ(T), Train, out4) <=> ((occupied(T, Train, out4) & ~(at(T, Train, out4) & ~at(succ(T), Train, out4))) | (at(T, Train, s4) & (gate(Train) = out4) & open(T, s4)))))).
...
fof(s1_occupied, axiom, (![T, Train]: (occupied(succ(T), Train, s1) <=> ((occupied(T, Train, s1) & ~(at(T, Train, s1) & ~at(succ(T), Train, s1))) | (at(T, Train, in) & (gate(Train) = out2) & open(T, in)) | (at(T, Train, in) & (gate(Train) = out3) & open(T, in)) | (at(T, Train, in) & (gate(Train) = out1) & open(T, in)))))).
...
fof(path_from_to_values, axiom, (![T, Train, N1, N2]: (path_free(T, Train, N1, N2) => (((N1 = s4) & (N2 = out4)) | ((N1 = in) & (N2 = out2)) | ((N1 = in) & (N2 = out3)) | ((N1 = in) & (N2 = out1)))))).

% No node can be occupied for given path.
fof(path_free_from_s4_to_out4, axiom, (![T, Train, OtherTrain]: (path_free(T, Train, s4, out4) <=> (at(T, Train, s4) & (gate(Train) = out4) & (~occupied(T, OtherTrain, s4) | (Train = OtherTrain)) & (~occupied(T, OtherTrain, out4) | (Train = OtherTrain)))))).
...
fof(open_s4, axiom, (![T]: (open(T, s4) <=> (?[Train]: (path_free(T, Train, s4, gate(Train))))))).
